...
const GREETING_TEMPLATES: Array<(name: string) => string> = [
  ...
  (n) => `Good afternoon, ${n}`,
  (n) => `Good evening, ${n}`,
  // Extra variety — keep casual, short, Gemini-style
  (n) => `What are we mocking today, ${n}?`,
  (n) => `${n}, got a ticket in mind?`,
  (n) => `Let's ship a mock, ${n}`,
  (n) => `Drop a ticket, ${n}`,
  (n) => `Your move, ${n}`,
  (n) => `Where to, ${n}?`,
  (n) => `Fresh start, ${n}?`,
  (n) => `Spin up a mock, ${n}`,
  (n) => `${n} — ready to design?`,
  (n) => `What needs a mock, ${n}?`,
  (n) => `Pick a Jira, ${n}`,
  (n) => `Let's dig in, ${n}`,
  (n) => `Onward, ${n}`,
  (n) => `Hey ${n}, what's cooking?`,
  (n) => `${n}, let's sketch something`,
  (n) => `Open a ticket, ${n}`,
  (n) => `What should we build, ${n}?`,
  (n) => `Time to mock, ${n}`,
  (n) => `Show me a ticket, ${n}`,
  (n) => `${n} — what's the brief?`,
  (n) => `Another one, ${n}?`,
  (n) => `Let's go, ${n}`,
  (n) => `Hey ${n}, which ticket?`,
  (n) => `${n}, fire when ready`,
  (n) => `What are we solving, ${n}?`,
];
