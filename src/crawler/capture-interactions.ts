 ...
 * are skipped via a text denylist.
 */
...
async function topVisibleOverlay(page: Page): Promise<VisibleOverlay | null> {
  return page.evaluate((overlaySel) => {
    ...
    const key = el.classList.contains("q-menu") ? "q-menu" : el.classList.contains("q-drawer") ? "q-drawer" : "q-dialog";
...
export async function captureInteractions(
  ...
  for (const handle of handles) {
    ...
    try {
      ...
      const label = ((await handle.innerText().catch(() => "")) || "").trim().replace(/\s+/g, " ").slice(0, 60);
      ...
      if (overlay && !seenSignatures.has(overlay.signature)) {
        ...
        out.push({ componentKey: overlay.componentKey, trigger: triggerText, html: overlay.html, signature: overlay.signature, screenshot });
      }
