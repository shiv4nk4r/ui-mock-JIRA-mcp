 ...
 * state file — for those, set CRAWL_USERNAME + CRAWL_PASSWORD and run `npm run crawl`.
 */
...
async function main() {
  ...
  if (!config.baseUrl) {
    console.error(
      "[crawl:login] CRAWL_BASE_URL is not set. Add it to .env.local, e.g.\n" +
  ...
  console.log(`[crawl:login] Opening ${config.baseUrl} (${automated ? "automated credential login" : "interactive login"})…`);
  ...
  if (automated) {
    ...
    console.log(ok
      ...
      : "[crawl:login] Automated login unconfirmed — saving state anyway.");
  } else {
  ...
  console.log(
    "[crawl:login] NOTE: if the app stores its token in sessionStorage, prefer inline login:\n" +
      "  set CRAWL_USERNAME + CRAWL_PASSWORD and run `npm run crawl`."
  );
