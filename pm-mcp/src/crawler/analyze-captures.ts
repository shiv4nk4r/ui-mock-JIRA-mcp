 ...
 *
 * Run: npm run analyze-captures -w pm-mcp
...
function extractRegions(html: string): { subNav: string; mainContent: string } {
  try {
    ...
    const tabs = root.querySelector(".q-tabs");
    const pageEl =
      root.querySelector(".q-page-container") ??
      root.querySelector(".q-page") ??
      root.querySelector("main");
    return {
      subNav: tabs?.toString() ?? "",
      mainContent: pageEl?.innerHTML ?? "",
    };
  } catch {
    return { subNav: "", mainContent: "" };
  }
}

function firstMatch(html: string, pattern: RegExp): string {
  const m = html.match(pattern);
  return m?.[0]?.slice(0, MAX_SNIPPET_CHARS) ?? "";
}

function processPage(page: CapturedPage, opts: AnalyzeOptions): ProcessedPage {
  let html = stripStyleAndScript(page.html);
  html = truncateTableBodies(html, opts.maxTableRows ?? MAX_TABLE_ROWS);
  if (opts.sanitize ?? page.sanitized) {
    html = sanitizeHtml(html);
  }

  const cssBundleId = page.cssBundleIds[0];
  html = injectTemplatePreviewHead(html, cssBundleId);

  const shell = extractShell(html);
  const { subNav, mainContent } = extractRegions(html);
  ...
  const meta: PageTemplateMeta = {
    ...
    subNavByteSize: Buffer.byteLength(subNav, "utf-8"),
    mainContentByteSize: Buffer.byteLength(mainContent, "utf-8"),
    cssBundleId,
  };
...
function buildAnalysisSummary(analysis: TemplateAnalysis): string {
  const lines = [
    ...
    `Pages analyzed: ${analysis.pageCount}`,
  ];
  for (const [archetype, info] of Object.entries(analysis.archetypes) as Array<
    [PageArchetype, TemplateAnalysis["archetypes"][PageArchetype]]
  >) {
    if (!info.routes.length) continue;
    lines.push(`- ${archetype}: ${info.routes.length} route(s), canonical=${info.canonicalRoute ?? "n/a"}`);
    if (info.commonPatterns.tableClasses) {
      lines.push(`  table: ${info.commonPatterns.tableClasses.slice(0, 200)}`);
    }
    if (info.commonPatterns.filterBarSnippet) {
      lines.push(`  filter: ${info.commonPatterns.filterBarSnippet.slice(0, 160)}`);
    }
  }
  return lines.join("\n");
}
...
export function analyzeCaptures(opts: AnalyzeOptions): TemplateAnalysis {
  ...
  if (!pages.length) {
    throw new Error(`No captured pages for label "${label}". Run crawl first.`);
  }

  const processed = pages.map((p) => processPage(p, opts));

  for (const { templateHtml, meta } of processed) {
    writePageTemplate(label, meta.slug, templateHtml, meta);
  }
  ...
  if (listingCanonical) {
    writeArchetypeTemplate(label, "listing-table", listingCanonical.templateHtml);
  }
  if (dashboardCanonical) {
    writeArchetypeTemplate(label, "dashboard-tabs", dashboardCanonical.templateHtml);
  }
  if (formCanonical) {
    writeArchetypeTemplate(label, "form", formCanonical.templateHtml);
  }
  ...
  syncPublicTemplateIndex(label, templateIndex);
  ...
  if (manifest) {
    ...
    writeManifest(label, {
      ...manifest,
      templatesGeneratedAt: analysis.generatedAt,
      archetypes,
  ...
  console.log(
    `[analyze] ${processed.length} template(s) → .cache/captures/${label}/templates/`
  );
...
/** Copy lightweight template index to pm-ui/public for static reference (no HTML duplication). */
function syncPublicTemplateIndex(label: string, index: TemplateIndex): void {
  try {
    const publicDir = path.join(PM_MCP_ROOT, "..", "pm-ui", "public", "captures", safeLabel(label));
    fs.mkdirSync(publicDir, { recursive: true });
    fs.writeFileSync(path.join(publicDir, "index.json"), JSON.stringify(index, null, 2), "utf-8");
  } catch {
    /* non-fatal — pm-ui reads from pm-mcp cache via filesystem fallback */
  }
}
